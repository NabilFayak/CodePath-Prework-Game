...
function updateTimer() {
  ...
  if(timeLeft > 0){
    ...
    stopGame();
    alert("Game Over. Looks like your time's run out!");
  }
}
...
function loseGame(){
  stopGame();
  alert("Game Over. Three Strikes, You're Out!");
}
...
function guess(btn){
  ...
  if(invalid_guesses < 2){
    if(pattern[guessCounter] == btn){
      ...
      //Guess was incorrect
      //Invalid guesses goes up by one!
      invalid_guesses++;
      alert(invalid_guesses + ' bad Guess(es)! ' + (3-invalid_guesses) + ' more to go!')
    }
  }else{
    //Guess was incorrect
    //GAME OVER: LOSE!
